...
const TestField = () => {
  ...
  const test: ITestResultForTeacher = useSelector((state: RootState) => state.testOfStudent.testResult);
  ...
  const ChangePoints = (event: any) => {
    const selectedQuestion: IQuestion | undefined = test.resultAnswers?.find((resultAnswer: IQuestion) => resultAnswer.selected === true);
    if (selectedQuestion) {
      dispatch(ChangeQuestionAttr(selectedQuestion.id, "points", parseInt(event.target.value)));
    }
  };
  ...
  const ChangeFeedback = (event: any) => {
    const selectedQuestion: IQuestion | undefined = test.resultAnswers?.find((resultAnswer: IQuestion) => resultAnswer.selected === true);
    if (selectedQuestion) {
      dispatch(ChangeQuestionAttr(selectedQuestion.id, "feedback", event.target.value));
    }
  };
  const handleBackClick = () => {
    const selectedQuestion: IQuestion | undefined = test.resultAnswers?.find((resultAnswer: IQuestion) => resultAnswer.selected === true);
    if (selectedQuestion && test.resultAnswers) {
      indexOfSelected = test.resultAnswers?.findIndex((resultAnswer: any) => resultAnswer.selected);
  ...
  const selectedQuestion: IQuestion | undefined = test && test.resultAnswers && test.resultAnswers.find((resultAnswer: IQuestion) => resultAnswer.selected === true);
  ...
  return (
    <>
      {selectedQuestion ? (
        <>
          ...
          {selectedQuestion?.type === 2 ? (
            <Card key={selectedQuestion.id}>
              <Card.Body>{selectedQuestion.answerOptions[0].text}</Card.Body>
            </Card>
          ) : (
          ...
          <Form.Control
            ...
            name="text"
            readOnly={userRole === 1}
            value={selectedQuestion.feedback !== undefined ? selectedQuestion.feedback : ""}
        ...
        <div>
          <h5 className="ms-1" style={{fontWeight: "bold", textAlign: "left", marginBottom: 10}}>
