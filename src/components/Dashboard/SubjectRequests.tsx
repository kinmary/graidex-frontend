...
const SubjectRequests = () => {
  ...
  const HandleAcceptClick = (e: any) => {
    ...
    dispatch(joinSubjectRequest(selectedSubjectId));
  };
  const HandleRejectClick = (e: any) => {
    const selectedSubjectId = e.currentTarget.id;
    dispatch(rejectSubjectRequest(selectedSubjectId));
  };
  ...
  return (
    <>
      <div style={{marginTop: "10px"}}>
        ...
        >
          <h5 style={{fontWeight: "bold", textAlign: "left", margin: 0}}>Subject Requests</h5>
        ...
        {subjectRequests.length > 0 ? (
          <Row xs={1} md={3} className="g-4">
            {subjectRequests.map((subject: any, idx: number) => (
              <Col key={idx}>
                <Card style={{textAlign: "left", width: "100%", height: 300}} id={subject.id}>
                  <Card.Img variant="top" src={subject.subjectInfo.imageUrl || logoDark} style={{height: "60%", objectFit: "cover"}} />
                  <Card.Body>
                    <Card.Subtitle className="mb-2 text-muted">{subject.subjectInfo.customId}</Card.Subtitle>
                    <Card.Title>{subject.subjectInfo.title}</Card.Title>
                    <Button id={subject.id} variant="outline-danger" style={{width: "49%"}} onClick={HandleRejectClick}>
                      <i className="bi bi-x-lg"></i> Reject
                    </Button>

                    <Button id={subject.id} style={{width: "49%", float: "right"}} onClick={HandleAcceptClick}>
          ...
          <Alert variant="primary" style={{textAlign: "center"}}>
            <h6>You don't have any pending subject requests.</h6>
          </Alert>
        )}
      </div>
    </>
