...
export const getSubjectRequests = () => {
  return async (dispatch: AppDispatch) => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/SubjectRequest/all`);
...
export const joinSubjectRequest = (subjectRequestId: string) => {
  return async (dispatch: AppDispatch) => {
    try {
      const response = await axios.post(`${API_BASE_URL}/api/SubjectRequest/join/` + subjectRequestId);
      if (response.status === 200) {
...
export const rejectSubjectRequest = (subjectRequestId: string) => {
  return async (dispatch: AppDispatch) => {
    try {
      const response = await axios.post(`${API_BASE_URL}/api/SubjectRequest/reject/` + subjectRequestId);
      if (response.status === 200) {
        dispatch(getSubjectRequests());
        dispatch(getAllSubjects());
      }
...
export const addStudent = (id: string, studentEmail: string) => {
  return async (dispatch: AppDispatch) => {
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/api/SubjectRequest/create/${subjectId}?studentEmail=` + studentEmail);
      if (response.status === 200) {
...
export const deleteSubjectRequest = (id: string, subjectRequestId: string) => {
  return async (dispatch: AppDispatch) => {
    try {
      let subjectId = parseInt(id);
      const response = await axios.delete(`${API_BASE_URL}/api/SubjectRequest/delete/${subjectRequestId}`);
      if (response.status === 200) {
        dispatch(getSubjRequestsOfTeacher(id));
        dispatch(getStudentsList(subjectId));
      }
...
export const getSubjRequestsOfTeacher = (id: string) => {
  return async (dispatch: AppDispatch) => {
    try {
      let subjectId = parseInt(id);
      const response = await axios.get(`${API_BASE_URL}/api/SubjectRequest/of-teacher/${subjectId}`);
      if (response.status === 200) {
