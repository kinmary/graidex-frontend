  ...
  return (
    ...
    >
      <Link to="/" style={{textDecoration: "none"}}>
        <Image src={theme === themes.light ? logo : logoDark} width="100" style={{marginRight: 10}} onClick={handleLogoClick} />
      </Link>
      <div style={{display: "block", marginTop: -5}}>
        <Badge pill bg={auth.userRole === 0 ? "primary" : "info"} style={{fontSize: 10}}>
      ...
      >
        <NavDropdown className="dropdown-no-after" align="end" id="basic-nav-dropdown" title={<Image className="profile-image" src={auth.profilePic || profilePic} />}>
          <NavDropdown.ItemText className="flex-nowrap">
            <Row className="flex-nowrap">
              ...
              <Col>
                ...
                <br />
                <span className="text-muted text-nowrap">{auth.email}</span>
              </Col>
            </Row>
          </NavDropdown.ItemText>
          <NavDropdown.Divider />
          <NavDropdown.Item as={Link} to={"/edit-profile"} onClick={handleEditProfile}>
            <i className="bi bi-gear me-2" style={{marginRight: 5}}></i>
            Settings
          </NavDropdown.Item>
          <NavDropdown.Item as="button" disabled>
            <i className="bi bi-translate me-2" style={{marginRight: 5}}></i>
            Language
          </NavDropdown.Item>
          <NavDropdown.Item as="button" onClick={handleThemeChange}>
