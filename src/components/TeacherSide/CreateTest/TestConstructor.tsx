    ...
    if (questions) {
      ...
      if (question) {
        ...
        if (event.target.name === "maxPoints") {
          dispatch(
            InputChange(id, event.target.name, parseInt(event.target.value))
          );
        } else {
  ...
  const handleQuestionDelete = () => {
    ...
    if (questions) {
      ...
      let updatedQuestions = questions.filter(
        (question: any) =>
          question.id.toString() !== selectedQuestion?.id.toString()
      );
                ...
                <Dropdown.Menu>
                  {questionTypes.map((type: any) => (
                    <Dropdown.Item
                      ...
                      onClick={() => {
                        dispatch(
                          InputChange(selectedQuestion.id, "type", type.value)
                        );
                        if (answerOptions.length > 0) {
                          let updated = selectedQuestion.answerOptions.map(
                            (answer: any) => {
                              return {
                                ...answer,
                                isCorrect: false,
